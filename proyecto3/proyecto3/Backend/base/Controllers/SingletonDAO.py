...
class SingletonDAO:
    ...
    Parte estudiantes --------------------------------------------------------------------------------------------------------------------------------------------------------------------
    @author: Andres
    @dev: Metodo del DAO que consulta de la base de datos un perfil de un estudiante
    @param: idestudiante, integer con el id del estudiante que se desea consultar
    @returns: informacion del estudiante
    """
    def consultarEstudiante(self, idestudiante):
        return {"estudiante": EstudianteSerializer(Estudiante.objects.filter(idestudiante=idestudiante),many=True).data}

    """
    @author: Andres
    @dev: Metodo del DAO que cambia el numero de telefono de un estudiante
    @param: idestudiante, integer con el id del estudiante que se desea consultar
            telefono, integer con el numero de telefono nuevo
    @returns: None
    """
    def cambiarTelefonoEstudiante(self, idestudiante, telefono):
        if (not Estudiante.objects.filter(idestudiante=idestudiante).exists()):
            return {"response": "existError"}
